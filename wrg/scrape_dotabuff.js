...
(async () => {
  ...
  // 1) Heroes list, year scope
  await page.goto('https://www.dotabuff.com/heroes?show=heroes&view=meta&mode=all-pick&date=year', { waitUntil: 'networkidle2' });
  const heroes = await page.evaluate(() => {
    const rows = Array.from(document.querySelectorAll('tr'));
    const list = [];
    for (const tr of rows) {
      const nameCell = tr.querySelector('a[href*="/heroes/"]');
      if (nameCell) list.push(nameCell.textContent.trim());
    }
    return list;
  });
  if (!heroes.length) throw new Error('Failed to read heroes list');

  // 2) Global WR per hero (same page, year scope)
  ...
  // 3) NxN matchups via counters?date=year
  ...
  async function processHero(item) {
    const url = `https://www.dotabuff.com/heroes/${item.slug}/counters?date=year`;
